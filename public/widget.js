/**
 * ClinicBridge AI Widget v2.1 — Production Build
 ...
 *   window.ClinicBridgeWidget.sendMessage(text)
 */
(function (w, d) {
  'use strict';
  ...
  const cfg = {
    ...
    lang: script?.dataset.lang || (navigator.language || 'en').slice(0, 2),
    disableBubbles: script?.dataset.disableBubbles === 'true',
    apiBase: 'https://app.clinicbridge-ai.com/api',
  };

  /* ── CONTENT ─────────────────────────────────────── */
  const WELCOME = {
    en: "Hello! I'm the AI assistant for this clinic. I can help you find the right treatment, answer your questions, or guide you to book an appointment. 😊",
    tr: "Merhaba! Bu kliniğin yapay zeka asistanıyım. Doğru tedaviyi bulmanıza, sorularınızı yanıtlamaya veya randevu almanıza yardımcı olabilirim. 😊",
  };

  const BUBBLES = {
    en: ['Need help choosing a treatment?', 'Ask me about our services', 'Want to book an appointment?', 'Have a question? Ask away!'],
    tr: ['Tedavi seçiminde yardım ister misiniz?', 'Hizmetlerimiz hakkında bilgi alın', 'Randevu almak ister misiniz?', 'Sorunuz mu var? Sorun!'],
  };

  const QUICK = {
    en: [
      { label: '📅 Book an appointment', msg: 'book' },
      { label: '🏥 What services do you offer?', msg: 'services' },
      { label: '💬 Contact on WhatsApp', msg: 'whatsapp' },
      { label: '📍 Where are you located?', msg: 'location' },
    ],
    tr: [
      { label: '📅 Randevu almak istiyorum', msg: 'book' },
      { label: '🏥 Hangi hizmetleri sunuyorsunuz?', msg: 'services' },
      { label: '💬 WhatsApp ile iletişim', msg: 'whatsapp' },
      { label: '📍 Neredesiniz?', msg: 'location' },
    ],
  };

  const RESPONSES = {
    en: {
      book: "I'd love to help you book! Please fill in our contact form or call us directly — our team will confirm your appointment within 24 hours. 📅",
      services: "We offer a wide range of treatments tailored to your needs. I recommend starting with a consultation so our specialists can recommend the best option for you. 😊",
      whatsapp: "You can reach our team directly via WhatsApp for quick assistance. 💬",
      location: "Please check our website for our exact address and opening hours. We look forward to welcoming you! 📍",
      default: [
        "Thank you for your message! I recommend speaking with one of our patient coordinators for personalized guidance. 😊",
        "Great question! Our team would be happy to help. Please contact us directly for the most accurate information.",
        "I'm here to help! For detailed information, please reach out to our team directly.",
      ],
    },
    tr: {
      book: "Randevu almak için iletişim formumuzu doldurabilir veya bizi doğrudan arayabilirsiniz — ekibimiz 24 saat içinde dönüş yapar. 📅",
      services: "İhtiyaçlarınıza özel geniş bir tedavi yelpazesi sunuyoruz. Uzmanlarımızın size en iyi seçeneği önerebilmesi için önce bir konsültasyonla başlamanızı tavsiye ederim. 😊",
      whatsapp: "Hızlı yardım için ekibimize doğrudan WhatsApp üzerinden ulaşabilirsiniz. 💬",
      location: "Adresimiz ve çalışma saatlerimiz için web sitemizi ziyaret edin. Sizi görmekten mutluluk duyarız! 📍",
      default: [
        "Mesajınız için teşekkürler! Kişisel rehberlik için hasta koordinatörlerimizden biriyle görüşmenizi tavsiye ederim. 😊",
        "Güzel soru! Ekibimiz yardımcı olmaktan mutluluk duyar. En doğru bilgi için lütfen doğrudan bize ulaşın.",
        "Yardımcı olmak için buradayım! Ayrıntılı bilgi için ekibimizle iletişime geçin.",
      ],
    },
  };
  ...
  const CSS = `
    ...
    #cbw-launcher{width:60px;height:60px;border-radius:18px;border:none;cursor:pointer;background:linear-gradient(145deg,#6366f1 0%,#4f46e5 100%);box-shadow:0 8px 28px rgba(99,102,241,.45);display:flex;align-items:center;justify-content:center;position:relative;transition:transform .25s,box-shadow .25s;flex-shrink:0;outline:none}
    #cbw-launcher:hover{transform:scale(1.08) translateY(-2px);box-shadow:0 12px 36px rgba(99,102,241,.55)}
    #cbw-launcher:focus-visible{outline:3px solid #a5b4fc;outline-offset:3px}
    ...
    .cbw-bubble{background:#fff;border:1px solid rgba(99,102,241,.15);border-radius:20px 20px 4px 20px;padding:10px 14px;font-size:13.5px;color:#1e293b;box-shadow:0 4px 18px rgba(0,0,0,.11);display:flex;align-items:center;gap:9px;max-width:236px;line-height:1.4;cursor:pointer;transition:box-shadow .2s,transform .2s;animation:cbw-bubble-in .35s cubic-bezier(.34,1.56,.64,1)}
    .cbw-bubble:hover{box-shadow:0 6px 22px rgba(0,0,0,.15);transform:translateX(-3px)}
    .cbw-bdot{width:8px;height:8px;border-radius:50%;background:#6366f1;flex-shrink:0}
    ...
    #cbw-head{background:linear-gradient(135deg,#6366f1 0%,#4f46e5 100%);padding:17px 20px;display:flex;align-items:center;justify-content:space-between;flex-shrink:0}
    ...
    .cbw-user .cbw-bubble-msg{background:linear-gradient(135deg,#6366f1,#4f46e5);color:#fff;border-radius:16px 4px 16px 16px}
    ...
    .cbw-qbtn{background:#EEF2FF;border:1.5px solid #E0E7FF;border-radius:10px;padding:9px 13px;font-size:13px;font-weight:500;color:#4f46e5;cursor:pointer;text-align:left;transition:all .2s;font-family:inherit}
    .cbw-qbtn:hover{background:#E0E7FF;border-color:#c7d2fe}
    ...
    #cbw-input:focus{border-color:#6366f1;box-shadow:0 0 0 3px rgba(99,102,241,.1)}
    #cbw-send{width:38px;height:38px;border-radius:10px;border:none;cursor:pointer;background:linear-gradient(135deg,#6366f1,#4f46e5);display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:transform .2s}
    ...
    #cbw-powered a{color:#6366f1;text-decoration:none;font-weight:600}
  ...
  function inject() {
    ...
    root.innerHTML += `
      <div id="cbw-panel" role="dialog" aria-label="ClinicBridge AI Assistant" aria-hidden="true">
        ...
        <div id="cbw-inputrow">
          <input id="cbw-input" type="text" placeholder="${lang === 'tr' ? 'Mesajınızı yazın…' : 'Type your message…'}" autocomplete="off"/>
    ...
    function appendMsg(text, isUser, container, isWelcome) {
      ...
      div.innerHTML = `<div class="cbw-bubble-msg">${text}</div><span class="cbw-ts">${isWelcome ? (lang === 'tr' ? 'Az önce' : 'Just now') : getTime()}</span>`;
    ...
    quickEl.querySelectorAll('.cbw-qbtn').forEach(btn => {
      btn.addEventListener('click', () => {
        send(btn.textContent, btn.dataset.key);
      });
    });
