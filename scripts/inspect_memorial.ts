...
const html = fs.readFileSync("memorial_hospital_source.html", "utf8");
...
$("h4").each((i, el) => {
  ...
  if (text && !text.includes("Health and Travel") && !text.includes("Your Health")) {
      let docBlock = $(el).closest('.doctor-profile, .doctor-card, div'); // approximation
      let pTexts: string[] = [];
      $(el).nextUntil('h4, h3').each((j, sib) => {
          pTexts.push($(sib).text().trim());
      });
      doctors.push({
          name: text,
          meta: pTexts.join(" ").substring(0, 50)
      });
  }
});
