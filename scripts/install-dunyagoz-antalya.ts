...
const CLINIC_URL = "https://feelinhealthy.com/medicalcenter/dunyagoz-antalya";
const STABLE_KEY = "dunyagoz_antalya";
const SLUG = "dunyagoz-antalya";
...
async function fetchAndParseSource() {
  ...
  if (!h1Lower.includes("dunyagoz") && !h1Lower.includes("dünyagöz") && !h1Lower.includes("antalya")) {
  ...
  $("h4").each((i, el) => {
    ...
    if (text && (text.includes("Dr.") || text.includes("Prof."))) {
        ...
        let cleanName = namePart.replace(/(Assoc\. Prof\.|Prof\. Dr\.|Prof\.|Opr\. Dr\.|Op\. Dr\.|Dr\.)/g, '').trim();
        let titlePart = "";
        let academic = "";
        if (namePart.includes("Assoc. Prof.")) { titlePart = "Assoc. Prof."; academic = "Assoc. Prof."; }
        else if (namePart.includes("Prof. Dr.")) { titlePart = "Prof. Dr."; academic = "Prof."; }
...
async function run() {
  ...
  console.log(`=== DUNYAGOZ ANTALYA INSTALLATION [${isApply ? "APPLY" : "DRY-RUN"}] ===\n`);
  ...
  allClinicsSnap.forEach(d => {
      ...
      if (d.id === "KY7x141fXMg5oIWHjBnQ" || slug === "dunyagoz-atakoy" || stable === "dunyagoz_atakoy") previousFound = true;
  });

  if (!previousFound) {
      console.error("previous_clinic_not_completed: Dünyagöz Ataköy is not found in the DB.");
      process.exit(1);
  } else {
      console.log("Previous clinic (Dünyagöz Ataköy) is verified as completed.");
  }
  ...
  for (const doc of allClinicsSnap.docs) {
    ...
    if (nName.includes("dünyagöz") || nName.includes("dunyagoz")) {
        if (nName.includes("antalya") || data.branch === "Antalya") {
  ...
  if (!existingClinicId && duplicateTargetCandidates.length === 1) {
    ...
    if (c.name.toLowerCase().includes("antalya")) {
  ...
  const clinicData = {
    ...
    branch: "Antalya",
    facilityName: "Dünyagöz Antalya",
    displayNameTr: "Dünyagöz Antalya",
    displayNameEn: "Dünyagöz Antalya",
    ...
    aliases: [
      "Dünyagöz Antalya",
      "Dunyagoz Antalya",
      "Dünyagöz Antalya Hastanesi",
      "Dünyagöz Antalya Hospital",
      "Dünyagöz Eye Hospital Antalya",
    ...
    location: {
      ...
      countryCode: "TR",
      city: "Antalya", 
      locationSummary: "Antalya, Türkiye",
      timezone: "Europe/Istanbul"
    },

    shortDescription: "Dünyagöz Antalya is a leading eye hospital in Antalya, Turkey.",
    shortOverviewTr: "Dünyagöz Antalya, Türkiye'nin Antalya ilinde önde gelen bir göz hastanesidir.",
    shortOverviewEn: "Dünyagöz Antalya is a leading eye hospital in Antalya, Turkey.",
    longDescription: overviewTextRaw || "Dünyagöz Antalya is a leading eye hospital in Antalya, Turkey.",
    fullOverviewTr: overviewTextRaw || "Dünyagöz Antalya, Türkiye'nin Antalya ilinde önde gelen bir göz hastanesidir.",
    fullOverviewEn: overviewTextRaw || "Dünyagöz Antalya is a leading eye hospital in Antalya, Turkey.",
  ...
  kbTopics.forEach(topic => {
    ...
    if (topic === "brand_and_branch") content = "Dünyagöz Hospitals Group, Antalya Branch";
    if (topic === "location_and_access") content = "Located in Antalya, Turkey.";
