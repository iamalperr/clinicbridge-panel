...
const CLINIC_URL = "https://feelinhealthy.com/medicalcenter/intermed-health-group-nisantasi";
const STABLE_KEY = "intermed_health_group_nisantasi";
const SLUG = "intermed-health-group-nisantasi";
...
async function fetchAndParseSource() {
  ...
  if (!h1Lower.includes("intermed") && !h1Lower.includes("nisantasi") && !h1Lower.includes("nişantaşı")) {
          ...
          if (pName && pPrice && pPrice !== "Price") {
              if (pPrice === "1.00€") {
                  prices.push({
                      ...
                      sourceDuration: pDur,
                      sourceCategory: "Medical Services"
                  });
              } else {
  ...
  $("h4").each((i, el) => {
    ...
    if (text && (text.includes("Dr.") || text.includes("Prof."))) {
        // Assume text is "Prof. Dr. M. Taner Gören"
        let cleanName = text.replace(/(Prof\. Dr\.|Prof\.|Dr\.)/g, '').trim();
        doctors.push({
            fullName: cleanName,
            normalizedFullName: cleanName.toLowerCase(),
            sourceDisplayName: text,
            title: text.includes("Prof.") ? "Prof. Dr." : "Dr.",
            academicTitle: text.includes("Prof.") ? "Prof." : "",
...
async function run() {
  ...
  console.log(`=== INTERMED HEALTH GROUP NISANTASI INSTALLATION [${isApply ? "APPLY" : "DRY-RUN"}] ===\n`);
  ...
  allClinicsSnap.forEach(d => {
      ...
      if (d.id === "MXcmnB56uwlAyJfDUifq" || slug === "neur-on-clinic" || stable === "neur_on_clinic") previousFound = true;
  ...
  if (!previousFound) {
      console.error("previous_clinic_not_completed: Neur-on Clinic is not found in the DB.");
      process.exit(1);
  } else {
      console.log("Previous clinic (Neur-on Clinic) is verified as completed.");
  }
  ...
  if (kadikoyFound) {
      ...
      console.error(`canonical_duplicate_existing_record_missing_or_invalid: Position 31 Check: Intermed Health Group | Kadikoy not found.`);
      process.exit(1);
  }
  ...
  for (const doc of allClinicsSnap.docs) {
    ...
    if (nName.includes("intermed") && (nName.includes("nisantasi") || nName.includes("nişantaşı") || data.branch === "Nişantaşı")) {
  ...
  if (!existingClinicId && duplicateTargetCandidates.length === 1) {
    ...
    if (c.name.toLowerCase().includes("nisantasi") || c.name.toLowerCase().includes("nişantaşı")) {
  ...
  const clinicData = {
    ...
    branch: "Nişantaşı",
    facilityName: "Intermed Health Group Nişantaşı",
    displayNameTr: "Intermed Health Group Nişantaşı",
    displayNameEn: "Intermed Health Group Nişantaşı",
    category: "multi_specialty_medical_center", 
    ...
    aliases: [
      ...
      "Intermed Nişantaşı",
      "Intermed Nisantasi",
      "Intermed Health Nişantaşı",
      "Intermed Nişantaşı Clinic"
    ],
    
    treatmentCategories: ["multi_specialty_medical_center"],
    
    location: {
      country: "Türkiye",
      countryCode: "TR",
      city: "İstanbul", 
      locationSummary: "Nişantaşı, İstanbul",
      timezone: "Europe/Istanbul"
    },

    shortDescription: "Intermed Health Group Nişantaşı is a multi-specialty medical center.",
    shortOverviewTr: "Intermed Health Group Nişantaşı, çok branşlı bir tıp merkezidir.",
    shortOverviewEn: "Intermed Health Group Nişantaşı is a multi-specialty medical center.",
    longDescription: overviewTextRaw || "Intermed Health Group Nişantaşı is a multi-specialty medical center.",
    fullOverviewTr: overviewTextRaw || "Intermed Health Group Nişantaşı, çok branşlı bir tıp merkezidir.",
    fullOverviewEn: overviewTextRaw || "Intermed Health Group Nişantaşı is a multi-specialty medical center.",
  ...
  if (!isApply) {
    ...
    console.log("Kadıköy-derived entity count: 0");
  ...
  const kbTopics = [
    ...
    "medical_departments", "check_up_services",
  ...
  kbTopics.forEach(topic => {
    ...
    if (topic === "brand_and_branch") content = "Intermed Health Group, Nişantaşı Branch";
    if (topic === "location_and_access") content = "Located in Nişantaşı, Istanbul, Turkey.";
    ...
    if (topic === "check_up_services") content = "Clinic provides various check-up packages.";
    if (topic === "medical_departments") content = "Clinic offers multi-specialty services.";
