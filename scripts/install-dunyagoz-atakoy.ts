...
const CLINIC_URL = "https://feelinhealthy.com/medicalcenter/dunyagoz-atakoy";
const STABLE_KEY = "dunyagoz_atakoy";
const SLUG = "dunyagoz-atakoy";
...
async function fetchAndParseSource() {
  ...
  if (!h1Lower.includes("dunyagoz") && !h1Lower.includes("dünyagöz") && !h1Lower.includes("atakoy") && !h1Lower.includes("ataköy")) {
...
async function run() {
  ...
  console.log(`=== DUNYAGOZ ATAKOY INSTALLATION [${isApply ? "APPLY" : "DRY-RUN"}] ===\n`);
  ...
  allClinicsSnap.forEach(d => {
      ...
      if (d.id === "XddoFGsDfWXtOzudfABU" || slug === "dunyagoz-hospitals-group-etiler" || stable === "dunyagoz_hospitals_group_etiler") previousFound = true;
  });

  if (!previousFound) {
      console.error("previous_clinic_not_completed: Dünyagöz Hospitals Group Etiler is not found in the DB.");
      process.exit(1);
  } else {
      console.log("Previous clinic (Dünyagöz Hospitals Group Etiler) is verified as completed.");
  }
  ...
  for (const doc of allClinicsSnap.docs) {
    ...
    if (nName.includes("dünyagöz") || nName.includes("dunyagoz")) {
        if (nName.includes("atakoy") || nName.includes("ataköy") || data.branch === "Ataköy") {
  ...
  if (!existingClinicId && duplicateTargetCandidates.length === 1) {
    ...
    if (c.name.toLowerCase().includes("atakoy") || c.name.toLowerCase().includes("ataköy")) {
  ...
  const clinicData = {
    ...
    branch: "Ataköy",
    facilityName: "Dünyagöz Ataköy",
    displayNameTr: "Dünyagöz Ataköy",
    displayNameEn: "Dünyagöz Ataköy",
    ...
    aliases: [
      "Dünyagöz Ataköy",
      "Dunyagoz Atakoy",
      "Dünyagöz Ataköy Hastanesi",
      "Dünyagöz Ataköy Hospital",
      "Dünyagöz Eye Hospital Ataköy",
      "Dunyagoz Eye Hospital Atakoy"
    ],
    ...
    location: {
      ...
      countryCode: "TR",
      city: "İstanbul", 
      locationSummary: "Ataköy, İstanbul",
      timezone: "Europe/Istanbul"
    },

    shortDescription: "Dünyagöz Ataköy is an eye hospital on Istanbul European Side.",
    shortOverviewTr: "Dünyagöz Ataköy, İstanbul Avrupa Yakası'nda yer alan bir göz hastanesidir.",
    shortOverviewEn: "Dünyagöz Ataköy is an eye hospital on Istanbul European Side.",
    longDescription: overviewTextRaw || "Dünyagöz Ataköy is an eye hospital on Istanbul European Side.",
    fullOverviewTr: overviewTextRaw || "Dünyagöz Ataköy, İstanbul Avrupa Yakası'nda yer alan bir göz hastanesidir.",
    fullOverviewEn: overviewTextRaw || "Dünyagöz Ataköy is an eye hospital on Istanbul European Side.",
    
    pricingDisclaimerTr: "Listelenen tutarlar FeelinHealthy profilindeki bilgilere dayanır. Kesin fiyat; göz muayenesi, gerekli tetkikler, tedaviye uygunluk değerlendirmesi ve kişisel tedavi planı sonrasında hastane tarafından teyit edilir.",
    pricingDisclaimerEn: "The listed amounts are based on information shown on the FeelinHealthy profile. The final price is confirmed by the hospital after an eye examination, required diagnostic tests, treatment suitability assessment and a personalized treatment plan.",
  };
  ...
  if (!isApply) {
    ...
    console.log("Etiler-derived entity count: 0");
  ...
  kbTopics.forEach(topic => {
    ...
    if (topic === "brand_and_branch") content = "Dünyagöz Hospitals Group, Ataköy Branch";
    if (topic === "location_and_access") content = "Located in Ataköy, Istanbul, Turkey.";
