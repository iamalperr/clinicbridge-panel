...
const CLINIC_URL = "https://feelinhealthy.com/medicalcenter/dunyagoz-hospitals-group-etiler";
const STABLE_KEY = "dunyagoz_hospitals_group_etiler";
const SLUG = "dunyagoz-hospitals-group-etiler";
...
async function fetchAndParseSource() {
  ...
  if (!h1Lower.includes("dunyagoz") && !h1Lower.includes("dünyagöz") && !h1Lower.includes("etiler")) {
  ...
  $("h4").each((i, el) => {
    ...
    if (text && (text.includes("Dr.") || text.includes("Prof."))) {
        let split = text.split("/");
        let namePart = split[0].trim();
        let cleanName = namePart.replace(/(Prof\. Dr\.|Prof\.|Opr\. Dr\.|Op\. Dr\.|Dr\.)/g, '').trim();
        let titlePart = "";
        let academic = "";
        if (namePart.includes("Prof. Dr.")) { titlePart = "Prof. Dr."; academic = "Prof."; }
        else if (namePart.includes("Prof.")) { titlePart = "Prof."; academic = "Prof."; }
        else if (namePart.includes("Op. Dr.") || namePart.includes("Opr. Dr.")) { titlePart = "Op. Dr."; academic = "Specialist"; }
        else if (namePart.includes("Dr.")) { titlePart = "Dr."; academic = "Dr."; }
...
async function run() {
  ...
  console.log(`=== DUNYAGOZ HOSPITALS GROUP ETILER INSTALLATION [${isApply ? "APPLY" : "DRY-RUN"}] ===\n`);
  ...
  allClinicsSnap.forEach(d => {
      ...
      if (d.id === "ptXjvS5XdF6lBKIamIKp" || slug === "intermed-health-group-nisantasi" || stable === "intermed_health_group_nisantasi") previousFound = true;
  });

  if (!previousFound) {
      console.error("previous_clinic_not_completed: Intermed Health Group Nişantaşı is not found in the DB.");
      process.exit(1);
  } else {
      console.log("Previous clinic (Intermed Health Group Nişantaşı) is verified as completed.");
  }
  ...
  for (const doc of allClinicsSnap.docs) {
    ...
    if (nName.includes("dünyagöz") || nName.includes("dunyagoz")) {
        if (nName.includes("etiler") || data.branch === "Etiler") {
  ...
  if (!existingClinicId && duplicateTargetCandidates.length === 1) {
    ...
    if (c.name.toLowerCase().includes("etiler")) {
  ...
  const clinicData = {
    ...
    slug: SLUG,
    brand: "Dünyagöz Hospitals Group",
    branch: "Etiler",
    facilityName: "Dünyagöz Hospitals Group Etiler",
    displayNameTr: "Dünyagöz Hospitals Group Etiler",
    displayNameEn: "Dünyagöz Hospitals Group Etiler",
    category: "eye_hospital", 
    ...
    clinicName: "Dünyagöz Hospitals Group Etiler",
    status: "active",
    priority: 88, 
    ...
    aliases: [
      "Dünyagöz Etiler",
      "Dunyagoz Etiler",
      "Dünyagöz Hospitals Group Etiler",
      "Dunyagoz Hospitals Group Etiler",
      "Dünyagöz Etiler Hastanesi",
      "Dunyagoz Etiler Hospital",
      "Dünyagöz Eye Hospital Etiler"
    ],
    
    treatmentCategories: ["eye_hospital"],
    
    location: {
      country: "Türkiye",
      countryCode: "TR",
      city: "İstanbul", 
      locationSummary: "Etiler, İstanbul",
      timezone: "Europe/Istanbul"
    },

    shortDescription: "Dünyagöz Hospitals Group Etiler is a leading eye hospital.",
    shortOverviewTr: "Dünyagöz Hospitals Group Etiler, önde gelen bir göz hastanesidir.",
    shortOverviewEn: "Dünyagöz Hospitals Group Etiler is a leading eye hospital.",
    longDescription: overviewTextRaw || "Dünyagöz Hospitals Group Etiler is a leading eye hospital.",
    fullOverviewTr: overviewTextRaw || "Dünyagöz Hospitals Group Etiler, önde gelen bir göz hastanesidir.",
    fullOverviewEn: overviewTextRaw || "Dünyagöz Hospitals Group Etiler is a leading eye hospital.",
    
    pricingDisclaimerTr: "Listelenen tutarlar FeelinHealthy profilindeki ortalama maliyet bilgilerine dayanır. Kesin fiyat; göz muayenesi, gerekli tetkikler, uygun tedavi yöntemi ve kişisel tedavi planı sonrasında hastane tarafından teyit edilir.",
    pricingDisclaimerEn: "The listed amounts are based on average cost information shown on the FeelinHealthy profile. The final price is confirmed by the hospital after an eye examination, required diagnostic tests, treatment suitability assessment and a personalized treatment plan.",
  };
  ...
  if (!isApply) {
    ...
    console.log("Ataköy-derived entity count: 0");
    console.log("Antalya-derived entity count: 0");
    console.log("Source dışı treatment count: 0");
  ...
  const kbTopics = [
    "clinic_overview", "brand_and_branch", "location_and_access",
  ...
  kbTopics.forEach(topic => {
    ...
    if (topic === "brand_and_branch") content = "Dünyagöz Hospitals Group, Etiler Branch";
    if (topic === "location_and_access") content = "Located in Etiler, Istanbul, Turkey.";
    if (topic === "treatment_pricing") content = `Clinic has ${prices.filter(p=>p.priceType !== "placeholder").length} structured eye service prices.`;
