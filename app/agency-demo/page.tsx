...
export default function AgencyDemoPage() {
        ...
        try {
          ...
          fetch("/api/public/agency/feelinhealthy/lead", {
            ...
            body: JSON.stringify({
              ...
              language: lang,
              treatmentCategory: getAgencyTreatmentContext(ctx).category || "other",
              treatmentSubcategory: getAgencyTreatmentContext(ctx).subcategory || "",
              ...
              selectedCity: getAgencySelectedCity(ctx),
              istanbulSide: getAgencyIstanbulSide(ctx) || undefined,
              travelDate: getAgencyTravelDate(ctx),
              source: "widget",
  ...
  const sendSideSelectionAction = async (side: string, optionId: string, actionType?: "confirm" | "reject") => {
    ...
    setAiMessages((prev) => [
      ...prev.map((m) => (m.type === "side_clarification" || m.type === "side_clarification_single")
    ...
    try {
      await fetch("/api/public/agency/feelinhealthy/lead", {
        ...
        body: JSON.stringify({
          ...
          language: lang,
          treatmentCategory: getAgencyTreatmentContext(sessionCtx).category || "other",
          ...
          selectedCity: getAgencySelectedCity(sessionCtx),
          istanbulSide: getAgencyIstanbulSide(sessionCtx) || undefined,
          travelDate: getAgencyTravelDate(sessionCtx),
              ...
              {aiMessages.map((msg) => (
                <div key={msg.id} style={{
                  ...
                  <div style={{ maxWidth: "88%", minWidth: 0 }}>
                    {/* Text bubble or Consent Card or Side Clarification Card */}
                    {msg.type === "consent_request" || msg.type === "consent_request_resolved" ? (
                      ...
                      <div style={{
                        ...
                        <IstanbulSideClarificationCard
                          type={msg.sideClarificationCard?.type || msg.type}
                          title={msg.sideClarificationCard?.title}
                          message={msg.text || msg.sideClarificationCard?.message}
                          options={msg.sideClarificationCard?.options || []}
                          lang={lang}
                          isResolved={msg.type === "side_clarification_resolved"}
                          selectedOptionId={msg.selectedSideOptionId}
                          disabled={aiTyping}
